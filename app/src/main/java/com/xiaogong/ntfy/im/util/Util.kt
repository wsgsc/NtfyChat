...
import com.xiaogong.ntfy.im.db.ACTION_PROGRESS_FAILED
import com.xiaogong.ntfy.im.db.ACTION_PROGRESS_ONGOING
import com.xiaogong.ntfy.im.db.ACTION_PROGRESS_SUCCESS
...
import com.xiaogong.ntfy.im.msg.MESSAGE_ENCODING_BASE64
...
fun formatMessage(notification: Notification): String {
    val decodedMessage = if (notification.title != "") {
    ...
    // Prepend username if available
    return if (notification.senderUser.isNotEmpty()) {
        "${notification.senderUser}: $decodedMessage"
    } else {
        decodedMessage
    }
}
...
fun fileStat(context: Context, contentUri: Uri?): FileInfo {
    ...
    return cursor.use { c ->
        ...
        if (size == 0L) {
            // Content provider URIs (e.g. content://com.xiaogong.ntfy.im.provider/cache_files/DQ4o7DitZAmw) return an entry, even
