package com.xiaogong.ntfy.im.db
...
import com.xiaogong.ntfy.im.service.NotAuthorizedException
import com.xiaogong.ntfy.im.service.WebSocketNotSupportedException
import com.xiaogong.ntfy.im.service.hasCause
...
data class Subscription(
    ...
    @ColumnInfo(name = "encryptionEnabled") val encryptionEnabled: Boolean, // Whether encryption is enabled for this subscription
    @ColumnInfo(name = "username") val username: String?, // Per-subscription username (for encrypted messages)
    @ColumnInfo(name = "encryptPassword") val encryptPassword: String?, // Per-subscription encryption password
...
data class Notification(
    ...
    @ColumnInfo(name = "senderUser") val senderUser: String = "", // Username of the sender (from encrypted message)
    ...
    constructor(
        ...
        notificationId, priority, tags, click, icon, actions, attachment, deleted, senderUser, event = ApiService.EVENT_MESSAGE
    )
}
...
abstract class Database : RoomDatabase() {
    ...
    companion object {
        ...
        private val MIGRATION_18_19 = object : Migration(18, 19) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE Subscription ADD COLUMN username TEXT")
                db.execSQL("ALTER TABLE Subscription ADD COLUMN encryptPassword TEXT")
            }
        }

        private val MIGRATION_19_20 = object : Migration(19, 20) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE Notification ADD COLUMN senderUser TEXT NOT NULL DEFAULT ''")
            }
        }
...
interface SubscriptionDao {
    @Query("""
        SELECT
          s.id, s.baseUrl, s.topic, s.instant, s.mutedUntil, s.minPriority, s.autoDelete, s.insistent, s.lastNotificationId, s.icon, s.upAppId, s.upConnectorToken, s.displayName, s.dedicatedChannels, s.encryptionEnabled, s.username, s.encryptPassword,
