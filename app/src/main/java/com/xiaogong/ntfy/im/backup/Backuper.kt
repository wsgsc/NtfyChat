package com.xiaogong.ntfy.im.backup
...
class Backuper(val context: Context) {
    ...
    private suspend fun applySubscriptions(subscriptions: List<Subscription>?) {
        ...
        subscriptions.forEach { s ->
            try {
                // Add to database
                val subscription = com.xiaogong.ntfy.im.db.Subscription(
                    id = s.id,
                    baseUrl = s.baseUrl,
                    topic = s.topic,
                    instant = s.instant,
                    ...
                    encryptionEnabled = s.encryptionEnabled ?: false,
                    username = s.username,
                    encryptPassword = s.encryptPassword,
                )
        ...
        notifications.forEach { n ->
            try {
                val actions = if (n.actions != null) {
                    n.actions.map { a ->
                        com.xiaogong.ntfy.im.db.Action(
                            id = a.id,
                            action = a.action,
                            label = a.label,
                            clear = a.clear,
                ...
                val attachment = if (n.attachment != null) {
                    com.xiaogong.ntfy.im.db.Attachment(
                ...
                repository.addNotification(com.xiaogong.ntfy.im.db.Notification(
                    id = n.id,
    ...
    private suspend fun applyUsers(users: List<User>?) {
        ...
        users.forEach { u ->
            try {
                repository.addUser(com.xiaogong.ntfy.im.db.User(
                    baseUrl = u.baseUrl,
                    username = u.username,
                    password = u.password
                ))
...
data class Subscription(
    ...
    @SerializedName("encryption_enabled") val encryptionEnabled: Boolean? = null,
    val username: String?,
    @SerializedName("encrypt_password") val encryptPassword: String?
)
...
data class Notification(
    ...
    val deleted: Boolean,
    @SerializedName("sender_user") val senderUser: String? = null // Username of the sender (nullable for backward compatibility)
)
